package manager
{
    ...
    public class GameAnimationPlayerBase extends Sprite
    {
        /**动画播放顺序类型：正序播放。 */
        ...
        public function GameAnimationPlayerBase()
        {
        ...
        public function play(start:* = 0, loop:Boolean = true, name:String = "", showWarn:Boolean = true):void
        {
            ...
            if (this.interval > 0)
            {
                frameLoop(1, this, _frameLoop, null, true);
        ...
        protected function _getFrameByLabel(label:String):int
        {
			for (var i:int = 0; i < _count; i++) {
				var item:* = _labels[i];
				if (item && (item as Array).indexOf(label) > -1) return i;
			}
            return 0;
        }
        ...
        {
            this._timeTotal += Laya.timer.delta;//Config.animationInterval;

            while (this._isPlaying && this._timeTotal >= this._interval)// && this._interval > 0)
            {
        ...
        public function _outFrameLoop(delta:Number):void
        {
            this._timeTotal += delta;
        ...
        /**@private */
        protected function _doFrame():void
        {
            if (_isReverse)
            {
                this._index--;
        ...
        public function _setControlNode(node:Sprite):void
        {
            if (_controlNode)
            {
				_controlNode.off(Event.DISPLAY, this, _resumePlay);
				_controlNode.off(Event.UNDISPLAY, this, _resumePlay);
            }
            _controlNode = node;
            if (node && node != this)
            {
				node.on(Event.DISPLAY, this, _resumePlay);
				node.on(Event.UNDISPLAY, this, _resumePlay);
            }
        }
		...
		protected function _resumePlay():void 
        {
            if (_isPlaying)
            {
                if (_controlNode.displayedInStage)
                {
                    play(_index, loop, _actionName);
                }
                else
                {
                    if (!outLoop)
        ...
        public function removeLabel(label:String):void
        {
            ...
            else if (_labels)
            {
                ...
                {
					_removeLabelFromList(_labels[name], label);
                }
            }
        }

        /**@private */
		private function _removeLabelFromList(list:Array, label:String):void 
        {
            if (!list) return;
         ...
         */
        public function clear():GameAnimationPlayerBase
        {
            stop();
            this._labels = null;
			return this;
