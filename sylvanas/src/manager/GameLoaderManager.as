package manager
{
     ...
     * @see laya.net.Loader
     */
    public class GameLoaderManager extends EventDispatcher// extends LoaderManager
    {
        /**@private */
        private static var _resMap:Object = {};
        /**@private */
        public static var createMap:Object = {atlas: [null, GameLoader.ATLAS]};
	...
	public function resetProgress():void {
        ...
        public function GameLoaderManager()
        {
		...
		private function _createOne(url:String, mainResou:Boolean, complete:Handler = null, progress:Handler = null, type:String = null, constructParams:Array = null, propertyParams:Object = null, priority:int = 1, cache:Boolean = true):void {
			url = URL.formatURL(url);//所有URL相关key存储和操作都用绝对路径
			var item:* = getRes(url);
			if (!item) {
				...
				var parserMap:Object = GameLoader.parserMap;
		 ...
		 */
		public function load(url:*, complete:Handler = null, progress:Handler = null, type:String = null, priority:int = 1, cache:Boolean = true, group:String = null, ignoreCache:Boolean = false, useWorkerLoader:Boolean = false):GameLoaderManager {
			if (url is Array) return _loadAssets(url as Array, complete, progress, type, priority, cache, group);
			
			var content:* = GameLoader.getRes(url);
		...
		private function _resInfoLoaded(original:String, complete:Handler):void {
			complete.runWith(GameLoader.getRes(original));
		}
		/**
		 * @private
		 */
		public function _createLoad(url:*, complete:Handler = null, progress:Handler = null, type:String = null, constructParams:Array = null, propertyParams:Object = null, priority:int = 1, cache:Boolean = true, ignoreCache:Boolean = false):GameLoaderManager {
        ...
        {
            this._loaderCount++;
            var loader:GameLoader = this._loaders.length ? this._loaders.pop() : new GameLoader();
            ...
            var _me:GameLoaderManager = this;
        ...
        private function _endLoad(resInfo:ResInfo, content:*):void
        {
            ...
            if (content == null)
            {
                ...
                if (errorCount < this.retryNum)
                {
					...
					GameLoader.clearRes(url);//使用create加载失败需要清除资源
			...
			if (resInfo.originalUrl) {
				content = GameLoader.getRes(resInfo.originalUrl);
			}
		...
		public function clearTextureRes(url:String):void {
			GameLoader.clearTextureRes(url);
		}
        ...
        {
            GameLoader.clearResByGroup(group);
        }
        ...
        private function _loadAssets(arr:Array, complete:Handler = null, progress:Handler = null, type:String = null, priority:int = 1, cache:Boolean = true, group:String = null):GameLoaderManager
        {
		...
		public function decodeBitmaps(urls:Array):void {
			...
			for (i = 0; i < len; i++) {
				var atlas:Array;
				atlas = GameLoader.getAtlas(urls[i]);
				if (atlas) {
