package manager
{
    ...
    public class GameLoader extends EventDispatcher
    {
		...
		private var _loadImgUrl:String = null;
		...
		public function load(url:String, type:String = null, cache:Boolean = true, group:String = null, ignoreCache:Boolean = false, useWorkerLoader:Boolean = false):void 
		{
			...
			if (type === IMAGE || type === "htmlimage" || type === "nativeimage") return _loadImage(getManifestUrl(url));
			...
			else 
			{
				...
				_http.send(getManifestUrl(url), null, "get", contentType);
			}
		}
        /**
         * 获取指定资源地址的数据类型。
        ...
        protected function getManifestUrl(url:String):String
        {
            if (url)
            {

                var maniFestDic:* = getRes(manifest_name);
                if (maniFestDic)
                {
                    var rawurl:String = url.replace(URL.basePath, "")
                    if (maniFestDic[rawurl])
                    {
                        return url.replace(rawurl, maniFestDic[rawurl]);
                    }
		...
		public static var localResDic:Array = [];
		
		//获取实际加载的url(主要是适配小游戏本地资源)
		public function getRealLoadUrl(url:String):String
		{
			if(url.indexOf("wxlocal") != -1)
			{
				return url;
			}
			var md5Url:String = url;
			if(manifest_name && manifest_name.length > 0)
			{
				if(url == manifest_name && localResDic && localResDic.indexOf(url) >= 0)
				{
					return "wxlocal/" + url;
				}
				var maniFestDic:* = getRes(manifest_name);
				if(maniFestDic)
				{
					md5Url = maniFestDic[url];
				}
			}
			if(localResDic && localResDic.indexOf(url) >= 0)
			{
				return "wxlocal/" + md5Url;
			}
			return url;
		}
        ...
        protected function onLoaded(data:* = null):void
        {
			...
			else if (type === ATLAS)
            {
				...
				if (!data.url && !data._setContext) 
                {
                    ...
                    return _loadImage(getManifestUrl(toloadPics.pop()));
                } 
				else
                {
                    if (_loadImgUrl)
                        cacheRes(_loadImgUrl, new Texture(data));
                    _data.pics.push(data);
                    if (_data.toLoads.length > 0)
                    {
                        event(Event.PROGRESS, 0.3 + 1 / _data.toLoads.length * 0.6);
                        //有图片未加载
                        return _loadImage(getManifestUrl(_data.toLoads.pop()));
                    }
                    ...
                    if (this._data.meta && this._data.meta.scale && this._data.meta.scale != 1)
                    {
						...
						if(this._data.after)
						{
							for(tmpIndex = 0; tmpIndex < frames.length; tmpIndex++)
							{
								tPic = pics[frames[tmpIndex][2]];//是否释放
								url = URL.formatURL(directory + frames[tmpIndex][0] + ".png");
								
								tPic.scaleRate = scaleRate;
								cacheRes(url, Texture.create(tPic, frames[tmpIndex][4], frames[tmpIndex][5], frames[tmpIndex][3], frames[tmpIndex][1], 
									frames[tmpIndex][8], frames[tmpIndex][9], frames[tmpIndex][7], frames[tmpIndex][6]));
...
//                    map.sort();
        ...
        {
            //Loader.setAtlasConfigs(url, config);
        }
